...
def _execute_one(
    ...
    if action.type == SuggestedActionType.UPDATE_TASK_PROGRESS:
        ...
        updated = update_task_progress(
            ...
            progress_percentage=float(action.payload["progressPercentage"]),
            note=str(action.payload.get("note") or "").strip() or None,
    ...
    if action.type == SuggestedActionType.CREATE_ISSUE:
        ...
        severity = IssueSeverity(str(action.payload.get("severity", "medium")).lower())
        issue = Issue(
            project_id=analysis.project_id, task_id=task.id if task else None,
            title=str(action.payload["title"]).strip(),
            description=str(action.payload["description"]).strip(),
            category=str(action.payload.get("category") or "").strip() or None,
            severity=severity, status=IssueStatus.OPEN, raised_by_id=current_user.id,
            affects_schedule=bool(action.payload.get("affectsSchedule", False)),
        )
        db.add(issue)
        db.flush()
    ...
    if action.type == SuggestedActionType.ADD_TASK_NOTE:
        ...
        note = TaskComment(
            ...
            content=str(action.payload["content"]).strip(),
        )
        db.add(note)
        db.flush()
    ...
    if action.type == SuggestedActionType.CREATE_SITE_REPORT_DRAFT:
        ...
        report = SiteReport(
            ...
            summary_text=str(action.payload["summaryText"]).strip(),
            work_completed=_join(action.payload.get("workCompleted")),
            delays=_join(action.payload.get("delays")),
            issues_summary=_join(action.payload.get("issues")),

...
export const AttachmentPanel = ({ projectId, entityType, entityId, initialCount = 0, readOnly = false }: {
  ...
  const load = async () => {
    try {
      ...
      toast.error(err?.response?.data?.detail || "Attachments could not be loaded.");
    }
  };
  const toggle = async () => {
  ...
  const upload = async (file?: File) => {
    ...
    catch (err: any) { toast.error(err?.response?.data?.detail || "File upload failed."); }
  ...
  return <div className="mt-3 border-t pt-3">
    ...
    {open && <div className="mt-3 space-y-2 rounded-lg bg-muted/20 p-3">
      {items.map((item) => <div key={item.id} className="flex items-center gap-2 text-xs">
        ...
        {!readOnly && <button aria-label="Delete attachment" onClick={async () => { try { await api.attachments.delete(item.id); await load(); toast.success("Attachment deleted."); } catch (err: any) { toast.error(err?.response?.data?.detail || "Attachment could not be deleted."); } }}><Trash2 size={13} /></button>}
      </div>)}
      {items.length === 0 && <p className="text-xs text-muted-foreground">No files attached to this event.</p>}

...
const ReadableRows = ({ value, quantities = false, units }: { value: Record<string, unknown>; quantities?: boolean; units?: Record<string, string> }) => {
  ...
  if (!Object.keys(value || {}).length) return <p className="text-sm text-muted-foreground">{t("ifcElements.not_defined_in_source_ifc")}</p>;
...
const ElementDetails = ({ element, nodes, issueCount, onFocus3D }: { element?: IFCElement; nodes: IFCSpatialNode[]; issueCount: number; onFocus3D?:(id:string)=>void }) => {
  const { t } = useTranslation();
  if (!element) return <EmptyState title={t("ifcElements.select_an_element")} description="Open an element to inspect identity, location, classification, materials, properties, quantities and relationships."/>;
  ...
  return <div className="space-y-4">
    <Card><div className="flex items-center justify-between"><h2 className="font-semibold">{t("ifcElements.identity")}</h2><Badge variant={issueCount ? "warning" : "success"}>{issueCount} issues</Badge></div><div className="mt-4">{fields([["Original IFC name", element.metadataJson?.originalName || element.name], ["Description", element.description], ["IFC class", element.entityType], ["Predefined type", element.predefinedType], ["Object type", element.objectType], ["Element type", element.typeName], ["Tag", element.tag], ["GlobalId", element.globalId]])}</div><Button className="mt-4" size="sm" onClick={()=>onFocus3D?.(element.id)}>{t("ifcElements.focus_in_3d")}</Button></Card>
    <Card><h2 className="font-semibold">{t("ifcElements.location")}</h2><div className="mt-4">{fields([["Building", nodeName(element.buildingNodeId)], ["Storey", nodeName(element.storeyNodeId)], ["Space", nodeName(element.spaceNodeId)], ["Zone", nodeName(element.zoneNodeId)], ["Coordinates", element.boundingBoxJson ? "Geometry bounds available" : undefined]])}</div></Card>
    <Card><div className="flex items-center justify-between"><h2 className="font-semibold">{t("ifcElements.classification")}</h2><SourceBadge source={element.metadataJson?.classifications?.[0]?.source || element.metadataJson?.disciplineSource}/></div><p className="mt-3 text-sm">{classificationLabel(element)}</p><div className="mt-3">{fields([["Discipline", labelize(element.discipline)], ["Confidence", `${Math.round((element.metadataJson?.disciplineConfidence || 0) * 100)}%`], ["Reason", element.metadataJson?.disciplineReason], ["Source attributes", element.metadataJson?.disciplineSourceAttributes?.join("; ")]])}</div></Card>
    <Card><h2 className="font-semibold">{t("ifcElements.materials")}</h2><p className="mt-3 text-sm">{materialStatus === "VIRTUAL_REFERENCE" ? "Virtual material reference" : materialStatus === "MISSING_FROM_SOURCE" || !element.materialSummary ? "Material not defined in source IFC — material data incomplete" : element.materialSummary}</p></Card>
    <Card><h2 className="font-semibold">{t("ifcElements.element_properties")}</h2><div className="mt-3"><ReadableRows value={element.propertiesJson || {}} units={element.metadataJson?.units}/></div></Card>
    <Card><h2 className="font-semibold">{t("ifcElements.element_quantities")}</h2><div className="mt-3"><ReadableRows value={element.quantitiesJson || {}} quantities units={element.metadataJson?.units}/></div></Card>
    <Card><h2 className="font-semibold">{t("ifcElements.relationships")}</h2><div className="mt-3">{fields([["Contained storey", nodeName(element.storeyNodeId)], ["Contained space", nodeName(element.spaceNodeId)], ["Connected systems", element.systemName], ["Type definition", element.typeName]])}</div></Card>
    <details className="rounded-lg border bg-card p-4"><summary className="cursor-pointer font-semibold">{t("ifcElements.developer_data")}</summary><pre className="mt-3 max-h-80 overflow-auto rounded bg-slate-950 p-3 text-xs text-slate-200">{JSON.stringify(element, null, 2)}</pre></details>
  </div>;
};
...
export const ElementsTab = ({ projectId, versionId, nodes, findings, initialElementIds, onFocus3D }: { projectId: string; versionId: string; nodes: IFCSpatialNode[]; findings: IFCFinding[]; initialElementIds?: string[]; onFocus3D?:(id:string)=>void }) => {
  ...
  return <div className="space-y-4"><Card>{!!linkedIds.length && <div className="mb-3 flex items-center justify-between rounded-lg bg-primary/5 p-3 text-sm"><span>{t("ifcElements.showing_elements_linked_from_a_finding")}</span><Button size="sm" variant="ghost" onClick={() => setLinkedIds([])}>{t("ifcElements.show_all_elements")}</Button></div>}<div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6"><div className="relative xl:col-span-2"><Search className="absolute left-3 top-3 text-muted-foreground" size={16}/><input aria-label={t("ifcElements.search_elements")} className="w-full rounded-md border bg-background py-2 pl-9 pr-3 text-sm" value={filters.q} onChange={(event) => patchFilter("q", event.target.value)} placeholder={t("ifcElements.search_name_type_tag_or_globalid")}/></div><select aria-label={t("ifcElements.discipline")} className={selectClass} value={filters.discipline} onChange={(event) => patchFilter("discipline", event.target.value)}><option value="">{t("ifcElements.all_disciplines")}</option>{options.disciplines.map((value) => <option key={value} value={value}>{labelize(value)}</option>)}</select><select aria-label={t("ifcElements.storey")} className={selectClass} value={filters.storeyId} onChange={(event) => patchFilter("storeyId", event.target.value)}><option value="">{t("ifcElements.all_storeys")}</option>{options.storeys.map((node) => <option key={node.id} value={node.id}>{node.name}</option>)}</select><select aria-label={t("ifcElements.ifc_class")} className={selectClass} value={filters.entityType} onChange={(event) => patchFilter("entityType", event.target.value)}><option value="">{t("ifcElements.all_ifc_classes")}</option>{options.classes.map((value) => <option key={value}>{value}</option>)}</select><select aria-label={t("ifcElements.completeness")} className={selectClass} value={filters.completeness} onChange={(event) => patchFilter("completeness", event.target.value)}><option value="">{t("ifcElements.all_completeness")}</option><option value="COMPLETE">{t("ifcElements.complete")}</option><option value="PARTIAL">{t("ifcElements.partial")}</option><option value="INCOMPLETE">{t("ifcElements.incomplete")}</option></select><input aria-label={t("ifcElements.element_type_filter")} className={selectClass} value={filters.elementType} onChange={(event) => patchFilter("elementType", event.target.value)} placeholder={t("ifcElements.element_type")}/><input aria-label={t("ifcElements.material_filter")} className={selectClass} value={filters.material} onChange={(event) => patchFilter("material", event.target.value)} placeholder={t("ifcElements.material")}/><input aria-label={t("ifcElements.system_filter")} className={selectClass} value={filters.system} onChange={(event) => patchFilter("system", event.target.value)} placeholder={t("ifcElements.system")}/><select aria-label={t("ifcElements.classification")} className={selectClass} value={filters.classification} onChange={(event) => patchFilter("classification", event.target.value)}><option value="">{t("ifcElements.all_classifications")}</option><option value="CLASSIFIED">{t("ifcElements.classified")}</option><option value="MISSING">{t("ifcElements.missing_classification")}</option></select><select aria-label={t("ifcElements.issue_severity")} className={selectClass} value={filters.issueSeverity} onChange={(event) => patchFilter("issueSeverity", event.target.value)}><option value="">{t("ifcElements.all_issue_severities")}</option>{["CRITICAL", "HIGH", "MEDIUM", "LOW", "INFORMATION"].map((value) => <option key={value}>{value}</option>)}</select></div><div className="mt-3 flex flex-wrap items-center justify-end gap-3"><div className="flex items-center gap-2 text-sm text-muted-foreground"><span>{visibleItems.length === items.length ? total.toLocaleString() : visibleItems.length.toLocaleString()} elements · {checked.size} selected</span><Button variant="outline" size="sm" disabled={!total} onClick={() => void exportRows()}><Download size={15}/> {t("ifcElements.export_filtered_results")}</Button></div></div></Card>
    {loading ? <LoadingState label={t("ifcElements.loading_elements")}/> : error ? <ErrorState message={error} onRetry={() => setReload((value) => value + 1)}/> : !visibleItems.length ? <EmptyState title={t("ifcElements.no_elements_match_these_filters")} description="Clear or change the filters to view extracted model elements."/> : <div className="grid gap-4 xl:grid-cols-[minmax(0,1.5fr)_minmax(360px,1fr)]"><Card padding="none"><div className="overflow-auto"><table className="min-w-[1450px] w-full text-left text-sm"><thead className="sticky top-0 bg-muted"><tr><th className="p-3">{t("ifcElements.select")}</th>{[["name", "Element name"], ["ifcClass", "IFC class"], ["type", "Element type"], ["discipline", "Discipline"], ["storey", "Storey"], ["space", "Space"], ["system", "System"], ["material", "Material"], ["classification", "Classification"], ["tag", "Tag / GlobalId"], ["issues", "Data / issues"]].map(([key, label]) => <th key={key} className="p-3"><button onClick={() => key !== "issues" && key !== "storey" && key !== "space" && key !== "system" && key !== "material" && key !== "classification" && setFilters((current) => ({ ...current, sortBy: key, sortDirection: current.sortBy === key && current.sortDirection === "asc" ? "desc" : "asc" }))}>{label}</button></th>)}</tr></thead><tbody className="divide-y">{visibleItems.map((item) => <tr key={item.id} className={`cursor-pointer hover:bg-muted/40 ${selected?.id === item.id ? "bg-primary/5" : ""}`} onClick={() => setSelected(item)}><td className="p-3"><button aria-label={`Select ${item.name}`} onClick={(event) => { event.stopPropagation(); toggle(item.id); }}>{checked.has(item.id) ? <CheckSquare size={17}/> : <Square size={17}/>}</button></td><td className="p-3"><b>{item.metadataJson?.originalName || item.name}</b>{!item.metadataJson?.originalName && <small className="block text-muted-foreground">{t("ifcElements.original_ifc_name_missing")}</small>}</td><td className="p-3">{item.entityType}</td><td className="p-3">{displayValue(item.typeName)}</td><td className="p-3"><Badge>{vocabulary.discipline(item.discipline)}</Badge></td><td className="p-3">{displayValue(nodes.find((node) => node.id === item.storeyNodeId)?.name)}</td><td className="p-3">{displayValue(nodes.find((node) => node.id === item.spaceNodeId)?.name)}</td><td className="p-3">{displayValue(item.systemName)}</td><td className="p-3">{displayValue(item.materialSummary)}</td><td className="p-3 max-w-52 truncate" title={classificationLabel(item)}>{classificationLabel(item)}</td><td className="p-3"><span>{displayValue(item.tag)}</span><small className="block max-w-32 truncate text-muted-foreground" title={item.globalId}>{item.globalId}</small></td><td className="p-3"><Badge variant={item.metadataJson?.completenessStatus === "COMPLETE" ? "success" : "warning"}>{labelize(item.metadataJson?.completenessStatus)}</Badge><small className="ml-2">{issueCounts.get(item.id) || 0}</small></td></tr>)}</tbody></table></div></Card><ElementDetails element={selected} nodes={nodes} issueCount={selected ? issueCounts.get(selected.id) || 0 : 0} onFocus3D={onFocus3D}/></div>}
  </div>;
};

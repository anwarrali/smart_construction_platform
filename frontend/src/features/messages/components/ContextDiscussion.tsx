...
export const ContextDiscussion = ({ projectId, contextType, contextId, title = "Discussion" }: Props) => {
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      if (!quiet) toast.error(err?.response?.data?.detail || "Unable to load discussion.");
  ...
  const send = async () => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.detail || "Discussion message could not be sent.");
  ...
  return <Card className="space-y-3">
    ...
    <div className="max-h-72 space-y-2 overflow-y-auto rounded border bg-muted/20 p-3">
      ...
      {!loading && !conversation?.messages.length && <p className="py-5 text-center text-sm text-muted-foreground">No discussion yet.</p>}
      {loading && <p className="py-5 text-center text-sm text-muted-foreground">Loading discussion…</p>}
    </div>

...
export const AIIntelligencePage=()=>{
  const {t}=useTranslation();const workspace=useProjectWorkspace();const params=useParams<{projectId?:string}>();const projectId=workspace.projectId||params.projectId;const navigate=useNavigate();const {role}=useRole();
  ...
  const action=async(id:string,operation:()=>Promise<unknown>,message:string)=>{setBusy(id);try{await operation();toast.success(message);await load();}catch(error){const detail=(error as {response?:{data?:{detail?:string}}}).response?.data?.detail;toast.error(detail||t("errors.reviewActionFailed"));}finally{setBusy("");}};
  ...
  return <div className="page-container space-y-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="flex items-center gap-2 text-sm font-semibold text-primary"><Sparkles size={16}/> {t("ai.subtitle")}</p><h1 className="mt-1 text-2xl font-bold">{t("ai.title")}</h1><p className="mt-1 text-sm text-muted-foreground">{t("ai.intro")}</p></div><Button disabled={busy==="run"} onClick={()=>void run()}><RefreshCw className={busy==="run"?"animate-spin":""} size={16}/> {t("ai.refreshAnalysis")}</Button></div>
    <Card className="border-amber-200 bg-amber-50/40"><div className="flex gap-3"><ShieldAlert className="text-amber-700"/><div><b>{t("ai.reviewRequired")}</b><p className="text-sm text-muted-foreground">{t("ai.reviewRequiredHint")}</p></div></div></Card>
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6"><Card><p className="text-xs text-muted-foreground">{t("ai.openInsights")}</p><p className="mt-2 text-3xl font-bold">{overview?.openInsights||0}</p></Card><Card><p className="text-xs text-muted-foreground">{t("ai.critical")}</p><p className="mt-2 text-3xl font-bold text-red-600">{overview?.severityCounts.CRITICAL||0}</p></Card><Card><p className="text-xs text-muted-foreground">{t("ai.warnings")}</p><p className="mt-2 text-3xl font-bold text-amber-600">{overview?.severityCounts.WARNING||0}</p></Card><Card><p className="text-xs text-muted-foreground">{t("ai.suggestions")}</p><p className="mt-2 text-3xl font-bold">{overview?.severityCounts.SUGGESTION||0}</p></Card><Card className="xl:col-span-2"><p className="text-xs text-muted-foreground">{t("ai.alignment")}</p><div className="mt-2 flex items-end gap-3"><p className="text-3xl font-bold">{alignment?.overall??0}%</p><Badge variant={(alignment?.overall||0)<50?"warning":"success"}>{(alignment?.overall||0)<50?t("ai.alignmentReview"):t("ai.alignmentAligned")}</Badge></div><p className="mt-2 text-xs text-muted-foreground">{t("ai.latestRevision",{revision:overview?.latestRevision?.revisionCode||overview?.latestRevision?.title||t("common.notAvailable")})}</p></Card></div>
    ...
    <Card><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">{t("ai.reviewQueue")}</h2><p className="text-sm text-muted-foreground">{t("ai.reviewQueueHint")}</p></div><div className="flex flex-wrap gap-2">
      ...
      <select className={selectClass} value={filters.category} onChange={e=>setFilters(v=>({...v,category:e.target.value}))} aria-label={t("common.category")}><option value="">{t("ai.allCategories")}</option>{categoryOptions.map(v=><option key={v} value={v}>{labelize(v)}</option>)}</select>
      {!!disciplineOptions.length&&<select className={selectClass} value={discipline} onChange={e=>setDiscipline(e.target.value)} aria-label={t("common.discipline")}><option value="">{t("ai.allDisciplines")}</option>{disciplineOptions.map(v=><option key={v} value={v}>{labelize(v)}</option>)}</select>}
    </div></div>
    ...
    {!visible.length?<Card><div className="py-10 text-center"><Bot className="mx-auto text-muted-foreground"/><p className="mt-3 font-semibold">{t("empty.noInsights")}</p><p className="text-sm text-muted-foreground">{t("ai.reviewQueueHint")}</p></div></Card>:<div className="grid gap-4 xl:grid-cols-2">{visible.map(item=><Card key={item.id} className={item.severity==="CRITICAL"?"border-red-300":item.severity==="WARNING"?"border-amber-300":""}><div className="flex items-start justify-between gap-3"><div className="flex gap-3">{item.category==="SUGGESTED_TASK"?<ClipboardList className="mt-1 text-primary"/>:<AlertTriangle className="mt-1 text-amber-600"/>}<div><div className="flex flex-wrap gap-2"><Badge variant={severityVariant(item.severity)}>{t("ai.severity."+item.severity,{defaultValue:labelize(item.severity)})}</Badge><Badge>{labelize(item.category)}</Badge><Badge variant="info">{t("ai.confidence",{value:Math.round(item.confidence*100)})}</Badge></div><h3 className="mt-3 text-lg font-bold">{item.title}</h3></div></div><Badge variant={["OPEN","NEW"].includes(item.status)?"warning":item.status==="RESOLVED"?"success":"neutral"}>{t("ai.reviewState."+item.status,{defaultValue:REVIEW_STATES.find(state=>state.value===item.status)?.label||labelize(item.status)})}</Badge></div><p className="mt-4 text-sm leading-6">{item.description}</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-lg bg-muted/40 p-3"><p className="text-xs font-semibold">{t("ai.whyDetected")}</p><p className="mt-1 text-xs text-muted-foreground">{item.reason}</p></div><div className="rounded-lg bg-primary/5 p-3"><p className="text-xs font-semibold">{t("ai.recommendedAction")}</p><p className="mt-1 text-xs text-muted-foreground">{item.recommendedAction}</p></div></div><details className="mt-3 rounded border p-3 text-xs"><summary className="cursor-pointer font-medium">{t("ai.sourceEvidence")}</summary><pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap">{JSON.stringify(item.evidenceJson,null,2)}</pre></details><div className="mt-4 flex flex-wrap gap-2">{!!item.affectedJson?.elements?.length&&<Button size="sm" variant="outline" onClick={()=>open3D(item)}><Eye size={15}/> {t("ai.viewIn3D")}</Button>}{["OPEN","NEW"].includes(item.status)&&<Button size="sm" variant="outline" disabled={busy===item.id} onClick={()=>void action(item.id,()=>api.aiIntelligence.review(projectId,item.id,"ACKNOWLEDGED"),"Insight acknowledged for review")}>{t("collaboration.acknowledge")}</Button>}<Button size="sm" variant="outline" disabled={busy===item.id} onClick={()=>void action(item.id,()=>api.aiIntelligence.review(projectId,item.id,"FALSE_POSITIVE"),"Insight dismissed")}>{t("ai.dismiss")}</Button><Button size="sm" variant="outline" disabled={busy===item.id} onClick={()=>void action(item.id,()=>api.aiIntelligence.review(projectId,item.id,"RESOLVED"),"Insight resolved")}><CheckCircle2 size={15}/> {t("ai.resolve")}</Button>{item.category==="SUGGESTED_TASK"&&role==="project_manager"&&!item.appliedEntityId&&<Button size="sm" disabled={busy===item.id} onClick={()=>void action(item.id,()=>api.aiIntelligence.createTask(projectId,item.id),"Task created for review")}>{t("ai.createTask")}</Button>}{item.category!=="SUGGESTED_TASK"&&!item.appliedEntityId&&["project_manager","engineer"].includes(role||"")&&<Button size="sm" disabled={busy===item.id} onClick={()=>void action(item.id,()=>api.aiIntelligence.createIssue(projectId,item.id),"Issue created")}>{t("ai.createIssue")}</Button>}</div></Card>)}</div>}
    {!!visible.length&&<Card><h2 className="font-semibold">{t("ai.sourcesUsed")}</h2><p className="text-sm text-muted-foreground">Open an insight below to see the current validity of every project record used.</p><div className="mt-3 space-y-2">{visible.map(item=><div key={item.id}><p className="text-xs font-medium">{item.title}</p><AIInsightSources projectId={projectId} insightId={item.id} fallback={item.evidenceJson}/></div>)}</div></Card>}
  </div>;
};

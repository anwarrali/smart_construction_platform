...
export const ConsultantReviewsPage = ({ history = false }: { history?: boolean }) => {
  ...
  const load = useCallback(async () => {
    ...
    } catch (err: any) { setError(err?.response?.data?.detail || "Unable to load reviews."); }
  ...
  return <div className="page-container space-y-6">
    ...
    <Card className="space-y-4"><div className="grid gap-3 md:grid-cols-[1fr_180px_190px]"><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search task, ID, description, or contractor engineer…" />
    ...
    {error && <Card><p className="text-destructive">{error}</p><Button className="mt-3" onClick={load}>Try Again</Button></Card>}
    <Card className="p-0 overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left text-sm"><thead><tr className="border-b text-muted-foreground"><th className="p-4">Task</th><th className="p-4">Submitted</th><th className="p-4">Review</th><th className="p-4">Impact</th><th className="p-4">Evidence</th><th className="p-4"></th></tr></thead><tbody>
      {items.map((item) => <tr key={item.id} className="border-b align-top last:border-0"><td className="p-4"><div className="flex flex-wrap gap-2"><p className="font-medium">{item.taskCode} · {item.taskTitle}</p>{item.isCritical && <Badge variant="danger">Critical</Badge>}</div><p className="mt-1 text-xs text-muted-foreground">{title(item.discipline)} · {title(item.priority)}</p>{item.description && <p className="mt-1 max-w-sm truncate text-xs text-muted-foreground">{item.description}</p>}</td>
        <td className="p-4"><p>{item.submittedBy?.fullName || "Contractor Engineer"}</p><p className="text-xs text-muted-foreground">{new Date(item.submittedAt).toLocaleString()}</p>{item.isResubmission && <Badge className="mt-1" variant="warning">Attempt {item.submissionNumber}</Badge>}</td>
        <td className="p-4"><Badge variant={item.reviewStatus === "approved" ? "success" : item.reviewStatus === "rejected" ? "danger" : item.reviewStatus === "clarification_requested" ? "warning" : "info"}>{title(item.reviewStatus)}</Badge>{item.reviewDueDate && <p className={`mt-2 text-xs ${item.isOverdue ? "text-red-600" : "text-muted-foreground"}`}>{item.isOverdue ? "Overdue · " : "Due · "}{item.reviewDueDate}</p>}</td>
        <td className="p-4">{item.blocksDependentWork ? <p className="flex items-center gap-1 text-amber-700"><GitBranch size={14}/> {item.dependentTasksBlocked} blocked</p> : <span className="text-muted-foreground">No downstream block</span>}{item.previousRejectionCount > 0 && <p className="mt-1 flex items-center gap-1 text-xs text-red-600"><AlertTriangle size={12}/>{item.previousRejectionCount} prior rejection(s)</p>}</td>
      ...
      {loading && <tr><td colSpan={6} className="p-10 text-center text-muted-foreground">Loading review submissions…</td></tr>}

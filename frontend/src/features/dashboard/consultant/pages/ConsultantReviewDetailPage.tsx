...
export const ConsultantReviewDetailPage = () => {
  ...
  return <div className="page-container space-y-6">
    <button className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground" onClick={() => navigate(workspace.path("reviews"))}><ArrowLeft size={16}/> {t("consultantReview.back_to_reviews")}</button>
    ...
    <div className="grid gap-6 xl:grid-cols-[1.5fr_1fr]"><div className="space-y-6"><Card><h2 className="font-semibold">{t("consultantReview.task_and_submission")}</h2><div className="mt-4 grid gap-4 sm:grid-cols-2 text-sm"><p><span className="block text-muted-foreground">{t("consultantReview.contractor_engineer")}</span>{data.review.submittedBy?.fullName || "—"}</p><p><span className="block text-muted-foreground">{t("consultantReview.submitted")}</span>{formatDateTime(data.review.submittedAt)}</p><p><span className="block text-muted-foreground">{t("consultantReview.execution_progress")}</span>{data.task.progressPercentage}%</p><p><span className="block text-muted-foreground">{t("consultantReview.planned_dates")}</span>{data.task.plannedStartDate || "—"} to {data.task.plannedEndDate || "—"}</p><p><span className="block text-muted-foreground">{t("consultantReview.priority")}</span>{title(data.task.priority)}</p><p><span className="block text-muted-foreground">{t("consultantReview.project_manager")}</span>{data.task.projectManager?.fullName || "—"}</p></div><p className="mt-4 text-sm">{data.task.description || "No task description."}</p>{data.review.completionNote && <div className="mt-4 rounded bg-muted/40 p-3 text-sm"><strong>{t("consultantReview.completion_note")}</strong> {data.review.completionNote}</div>}</Card>
      <Card><h2 className="flex items-center gap-2 font-semibold"><Paperclip size={17}/> Submitted Evidence ({data.submissionEvidence.length})</h2><div className="mt-3 space-y-2">{data.submissionEvidence.map((item) => <button key={item.id} type="button" onClick={() => { void downloadAuthedFile(`/attachments/${item.id}/download`, item.filename); }} className="flex w-full items-center justify-between rounded border p-3 text-left text-sm text-primary hover:bg-muted/30"><span>{item.filename}</span><span className="text-xs text-muted-foreground">{item.mime_type}</span></button>)}{!data.submissionEvidence.length && <p className="text-sm text-muted-foreground">{t("consultantReview.no_evidence_was_attached_to_this")}</p>}</div>{active && workspace.projectId && <AttachmentPanel projectId={workspace.projectId} entityType="TASK_REVIEW" entityId={data.review.id} />}</Card>

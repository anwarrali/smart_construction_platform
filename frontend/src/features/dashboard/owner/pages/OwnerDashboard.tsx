...
export const OwnerDashboard = () => {
  ...
  if (!projects.length) return <Card><div className="empty-state"><p className="empty-state-title">No assigned projects</p><p className="text-sm text-muted-foreground">An administrator must assign you to a project before executive data is available.</p></div></Card>;
  if (!data) return <Card><p className="text-sm text-destructive">{error || "Project data is unavailable."}</p></Card>;
    ...
    <div className="page-container space-y-6">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">Clear project health, schedule, cost, and decisions requiring attention.</p>
        </div>
        <Select
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Card className="p-5"><div className="flex justify-between"><div><p className="text-sm text-muted-foreground">Critical delayed tasks</p><p className="mt-2 text-3xl font-bold">{data.delayedTasks.length}</p><a href="#delays" className="text-xs font-medium text-primary">View critical delays</a></div><Clock className="text-rose-600" /></div></Card>
        <Card className="p-5"><div className="flex justify-between"><div><p className="text-sm text-muted-foreground">Open issues</p><p className="mt-2 text-3xl font-bold">{data.openIssues.length}</p><p className="text-xs text-muted-foreground">{data.openIssues.filter((item) => item.severity === "critical").length} critical</p></div><AlertTriangle className="text-amber-600" /></div></Card>
      </div>

      <div className="grid gap-6 xl:grid-cols-3">
        <Card className="xl:col-span-2 p-5">
          <h2 className="font-semibold">Attention required</h2>
          <p className="text-sm text-muted-foreground">Only major owner-relevant events are shown.</p>
          <div className="mt-4 space-y-3">
            ...
            {!data.attentionRequired.length && <div className="rounded-lg bg-emerald-50 p-4 text-sm text-emerald-700">No major items require owner attention.</div>}
          </div>
        </Card>

        <Card className="p-5">
          <div className="flex items-center gap-2"><Wallet size={18} /><h2 className="font-semibold">Project cost</h2></div>
          <div className="mt-5 space-y-4">
            <div><p className="text-xs text-muted-foreground">Overall budget</p><p className="text-2xl font-bold">{money(cost.budgetTotal)}</p></div>
            <div className="grid grid-cols-2 gap-3"><div><p className="text-xs text-muted-foreground">Spent</p><p className="font-semibold">{money(cost.budgetSpent)}</p></div><div><p className="text-xs text-muted-foreground">Remaining</p><p className="font-semibold">{money(remaining)}</p></div></div>
            <div><div className="mb-1 flex justify-between text-xs"><span>Cost progress</span><span>{cost.budgetTotal ? Math.min(Math.round(cost.budgetSpent / cost.budgetTotal * 100), 100) : 0}%</span></div><div className="h-2 overflow-hidden rounded-full bg-muted"><div className="h-full bg-emerald-600" style={{ width: `${cost.budgetTotal ? Math.min(cost.budgetSpent / cost.budgetTotal * 100, 100) : 0}%` }} /></div></div>
          </div>
        </Card>
      </div>

      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="p-5"><div className="flex items-center gap-2"><MessageSquareText size={18} /><h2 className="font-semibold">Your requests</h2></div><div className="mt-4 space-y-3">{data.pendingOwnerRequests.map((item) => <Link key={item.id} to={projectEntityPath(projectId, "OWNER_REQUEST", item.id, "owner")} className="block rounded-lg border p-3 transition-colors hover:bg-muted/40"><div className="flex justify-between gap-3"><p className="text-sm font-semibold">{item.title}</p><span className="text-xs font-medium capitalize">{item.status.replaceAll("_", " ").toLowerCase()}</span></div>{item.needsOwnerInput && <p className="mt-1 text-xs font-semibold text-amber-700">Your clarification is required.</p>}</Link>)}{!data.pendingOwnerRequests.length && <p className="text-sm text-muted-foreground">No active owner requests. New client requests and their engineering responses will appear here.</p>}</div><Link className="mt-3 inline-flex text-sm font-medium text-primary" to={projectModulePath(projectId, "requests", "owner")}>Open requests →</Link></Card>
        <Card className="p-5"><div className="flex items-center gap-2"><CalendarDays size={18} /><h2 className="font-semibold">Upcoming engineer visits</h2></div><div className="mt-4 space-y-3">{data.upcomingSiteVisits.map((visit) => <Link key={visit.id} to={projectEntityPath(projectId, "SITE_VISIT", visit.id, "owner")} className="block rounded-lg border p-3 transition-colors hover:bg-muted/40"><p className="text-sm font-semibold">{visit.title}</p><p className="text-xs text-muted-foreground">{new Date(visit.scheduledStart).toLocaleString()} · {visit.location || "Project site"}</p></Link>)}{!data.upcomingSiteVisits.length && <p className="text-sm text-muted-foreground">No engineer visits are currently scheduled.</p>}</div><Link className="mt-3 inline-flex text-sm font-medium text-primary" to={projectModulePath(projectId, "site-visits", "owner")}>Open schedule →</Link></Card>
        <Card className="p-5">
          ...
          <div className="mt-4 space-y-3">{data.milestones.map((milestone) => <div key={milestone.id} className="flex items-center justify-between border-b pb-3"><div><p className="text-sm font-medium">{milestone.name}</p><p className="text-xs text-muted-foreground">{milestone.plannedDate || "No planned date"}</p></div><span className="text-xs capitalize">{milestone.status}</span></div>)}{!data.milestones.length && <p className="text-sm text-muted-foreground">No milestones have been scheduled.</p>}</div>
        </Card>
        ...
        <Card className="p-5">
          ...
          <div className="mt-4 space-y-4">{data.projectBreakdown.map((phase) => <div key={phase.name}><div className="mb-1 flex justify-between text-sm"><span>{phase.name}</span><span>{phase.completionPercentage}%</span></div><div className="h-2 overflow-hidden rounded-full bg-muted"><div className="h-full bg-primary" style={{ width: `${phase.completionPercentage}%` }} /></div></div>)}{!data.projectBreakdown.length && <p className="text-sm text-muted-foreground">No project phases are available.</p>}</div>
        </Card>
      </div>

      <div id="delays" className="grid gap-6 lg:grid-cols-2">
        <Card className="p-5"><h2 className="font-semibold">Critical delays</h2><div className="mt-4 space-y-3">{data.delayedTasks.map((task) => <div key={task.id} className="flex justify-between rounded-lg border p-3"><div><p className="text-sm font-semibold">{task.name}</p><p className="text-xs text-muted-foreground">{task.taskCode}</p></div><span className="text-sm font-semibold text-rose-600">{task.daysDelayed} days</span></div>)}{!data.delayedTasks.length && <p className="text-sm text-muted-foreground">No critical tasks are delayed.</p>}</div></Card>
        <Card className="p-5"><h2 className="font-semibold">Open issues</h2><div className="mt-4 space-y-3">{data.openIssues.map((issue) => <div key={issue.id} className="rounded-lg border p-3"><div className="flex justify-between gap-3"><p className="text-sm font-semibold">{issue.title}</p><span className={`text-xs font-semibold capitalize ${issue.severity === "critical" ? "text-rose-600" : issue.severity === "high" ? "text-amber-600" : "text-muted-foreground"}`}>{issue.severity}</span></div><p className="mt-1 text-sm text-muted-foreground">{issue.summary}</p></div>)}{!data.openIssues.length && <p className="text-sm text-muted-foreground">No active issues.</p>}</div></Card>
      </div>
    </div>
  );
};

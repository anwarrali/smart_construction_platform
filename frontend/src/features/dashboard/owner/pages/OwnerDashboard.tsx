  ...
  return (
    <div className="page-container space-y-6">
      ...
      <Card className="border-primary/20 bg-primary/5 p-5">
        <div className="flex items-start gap-3"><CheckCircle2 className="mt-1 text-primary" /><div className="flex-1"><h2 className="font-semibold">{t("owner.sinceYourLastVisit")}</h2><p className="text-xs text-muted-foreground">{data.sinceLastVisit.basis === "YOUR_PREVIOUS_VISIT" && data.sinceLastVisit.previousVisitAt ? `Verified and approved project information recorded since you last opened this project on ${formatDateTime(data.sinceLastVisit.previousVisitAt)}.` : `First recorded visit — showing verified and approved project information from the last ${data.sinceLastVisit.periodDays} days.`}</p><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div><p className="text-sm font-bold">{data.sinceLastVisit.nextEngineerVisit ? formatDateTime(data.sinceLastVisit.nextEngineerVisit) : t("common.notScheduled")}</p><p className="text-xs text-muted-foreground">{t("owner.nextEngineerVisit")}</p></div>
        </div></div></div>
      </Card>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="p-5">
          ...
          <div className="mt-4 space-y-3">{data.milestones.map((milestone) => <div key={milestone.id} className="flex items-center justify-between border-b pb-3"><div><p className="text-sm font-medium">{milestone.name}</p><p className="text-xs text-muted-foreground">{milestone.plannedDate || t("common.notScheduled")}</p></div><span className="text-xs capitalize">{vocabulary.milestoneStatus(milestone.status)}</span></div>)}{!data.milestones.length && <p className="text-sm text-muted-foreground">{t("owner.noMilestones")}</p>}</div>
        </Card>
      ...
      <div id="delays" className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="p-5"><h2 className="font-semibold">{t("project.openIssues")}</h2><div className="mt-4 space-y-3">{data.openIssues.map((issue) => <div key={issue.id} className="rounded-lg border p-3"><div className="flex justify-between gap-3"><p className="text-sm font-semibold">{issue.title}</p><span className={`text-xs font-semibold capitalize ${issue.severity === "critical" ? "text-state-overdue" : issue.severity === "high" ? "text-state-review" : "text-muted-foreground"}`}>{vocabulary.severity(issue.severity)}</span></div><p className="mt-1 text-sm text-muted-foreground">{issue.summary}</p></div>)}{!data.openIssues.length && <p className="text-sm text-muted-foreground">{t("owner.noIssues")}</p>}</div></Card>
      </div>
    </div>
  );
};

    ...
    >
      <div className="space-y-2">
        ...
        {(onEdit || onDelete) && <div className="flex justify-end gap-2 border-t pt-2">
          {onEdit && <button type="button" className="text-xs font-medium text-primary hover:underline" onClick={(event) => { event.stopPropagation(); onEdit(task); }}>Edit</button>}
          {onDelete && <button type="button" className="text-xs font-medium text-destructive hover:underline" onClick={(event) => { event.stopPropagation(); onDelete(task); }}>Delete</button>}
        </div>}
      </div>
    </Card>
  );

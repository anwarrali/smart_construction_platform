...
class ConstructionFieldApp extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    // Push starts and stops in step with the session. Watched here, at the
    // root, because it must be alive for the whole app: registration happens
    // once after sign-in, and a notification tapped from a terminated app is
    // delivered during startup — long before any screen has been built.
    ref.watch(pushLifecycleProvider);
    return _buildApp(context, ref);
  }

  Widget _buildApp(BuildContext context, WidgetRef ref) => MaterialApp.router(
